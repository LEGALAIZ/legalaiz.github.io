	...
	this.create = function () {
		...
		if (Object.keys(catalog).length) {
			...
			var parseListHeader = function () {
				...
				var epgUrl = listCfg['url-tvg'] || listCfg['x-tvg-url'] || '';
				listCfg['epgCode'] = utils.hash36(epgUrl.toLowerCase().replace(/https:\/\//g, 'http://'));
				...
				if (/^https?:\/\/.+/i.test(epgUrl)) {
					channelsUri += '/' + listCfg['epgCode'] + '?url=' + encodeURIComponent(epgUrl)
						+ '&uid=' + utils.uid() + '&sig=' + generateSigForString(epgUrl);
				}
				network.silent(
					Lampa.Utils.protocol() + 'epg.rootu.top/api/' + channelsUri,
					function(d){
						chIDs = d;
			...
			var parseList = function () {
				...
				while (i < l.length) {
					...
					if (!!channel.Url && !channel.isYouTube) {
						...
						channel['epgId'] = listCfg['isEpgIt999']
							? (channel['tvg-id'] && /^\d+$/.test(channel['tvg-id']) ? channel['tvg-id'] : epgIdByName(channel['Title'], true))
							: (chIDs.id2epg[channel['tvg-id'] || ''] || channel['tvg-id'] || epgIdByName(channel['Title'], isSNG));
						if (isSNG && !/^\d+$/.test(channel['epgId'])) {
							channel['epgId'] = epgIdByName(channel['Title'], isSNG) || channel['epgId'];
						}
						if (!channel['tvg-logo']) {
							if (channel['epgId'] && (listCfg['isEpgIt999'] || isSNG) && /^\d+$/.test(channel['epgId'])) {
								channel['tvg-logo'] = Lampa.Utils.protocol() + 'epg.it999.ru/img2/' + channel['epgId'] + '.png'
							} else if (isSNG && channel['Title'] !== "Ch " + chNum) {
